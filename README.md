...
Nota implementativa: questa fase è implementata dal comando **`videodoc code`**. Il comando consuma solo i risultati già prodotti da `videodoc ocr` (non rilegge immagini e non carica RapidOCR), classifica ogni `ocr_text` in `plain_text`, `terminal_command`, `source_code`, `configuration`, `error_message`, `file_path` o `ui_label`, salva i soli blocchi code-like deduplicati in `workdir/<video_id>/code/<video_id>.json` e nella tabella `code_blocks` di `project.db` (§31), e aggiorna esclusivamente `frames.contains_code`. La deduplicazione usa testo normalizzato + hash SHA-256, preservando tutti i frame sorgente e i timestamp in cui lo stesso blocco è apparso. JSON, YAML e Python sono validati con parser reali; comandi terminale, path ed errori sono verificati con regole deterministicamente riproducibili; JavaScript/TypeScript/HTML/CSS/SQL/Dockerfile sono riconosciuti ma marcati per revisione in strict mode finché non esiste un parser dedicato. Ogni video genera anche `workdir/<video_id>/code/code_review_report.md` con i blocchi da controllare (confidenza OCR bassa, validazione fallita o blocco non verificabile). L'idempotenza confronta le impostazioni `code.*` e la firma completa degli input OCR (`frame_id`, timestamp, hash percettivo, hash del testo OCR e confidenza), così una riestrazione dei frame o un nuovo OCR non può riusare blocchi vecchi su contenuto cambiato. Vedi `docs/features/code-extraction.md`.
...
Nota implementativa: questa fase è implementata dai comandi **`videodoc ask`**, **`videodoc chat`** e **`videodoc index-docs`**. `ask` supporta `--source docs|raw|hybrid`, filtri `--video`, `--from`, `--to` e usa la documentazione generata come fonte predefinita indicizzandola in `indexes/documentation_index.json` con `source_type = generated_documentation`; `index-docs` espone la stessa indicizzazione come comando esplicito. Se i Markdown non esistono ancora ma esistono indici raw/codebase, la pipeline resta interrogabile usando chunk originali o snippet della codebase. `chat` salva sessioni in `project.db` (`chat_sessions`, `chat_messages`) e in `sessions/<session_id>.json`, includendo la cronologia recente nelle domande successive. Le risposte restano estrattive e locali, con regola anti-allucinazione: se le fonti non supportano una risposta, lo dichiarano. Vedi `docs/features/chat.md`.
