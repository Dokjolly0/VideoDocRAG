...
def embed_command(
    ...
    workers: int | None = typer.Option(None, "--workers", min=1, help="Number of videos to embed concurrently."),
) -> None:
    try:
        service = ProjectService.load(project)
        with RichProgressReporter(console) as reporter:
            result = EmbeddingService(service.project_dir, service.config, workers_override=workers).run(progress=reporter)
    except (
        ProjectNotFoundError,
        InvalidConfigError,
        NoVideosFoundError,
