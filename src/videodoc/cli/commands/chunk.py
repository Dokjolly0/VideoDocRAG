...
def chunk_command(
    ...
    workers: int | None = typer.Option(None, "--workers", min=1, help="Number of videos to chunk concurrently."),
) -> None:
    try:
        service = ProjectService.load(project)
        with RichProgressReporter(console) as reporter:
            result = ChunkingService(service.project_dir, service.config, workers_override=workers).run(progress=reporter)
