...
Only `id`/`video_id`/`timestamp_seconds`/`image_path`/`perceptual_hash` are populated by this phase. `ocr_text`/`ocr_confidence` stay `NULL` until `videodoc ocr` runs (README §19, see [features/ocr.md](ocr.md)); `contains_code` stays `0` until the not-yet-implemented code-detection phase (README §20) fills it in.
...
- The self-heal path (`_process_existing`) preserves each frame row's `ocr_text`/`ocr_confidence`/`contains_code` by reading the current DB values first and carrying them over for matching frame ids, rather than rebuilding rows from `frames.json` alone (which never carries those three columns at all) — found by code review once `videodoc ocr` (README §19) existed: a plain idempotent `videodoc frames` rerun was silently wiping any OCR results already written for the exact same frame ids, since `replace_frames` is a full DELETE+INSERT and the manifest has nothing to repopulate them with. Verified end-to-end: OCR text/confidence survive a `videodoc frames` rerun after `videodoc ocr` has run.
