...
- `DEFAULT_EXCLUDES` (README §8.3, 23 entries) plus `resolve_excludes()`/`split_excludes()` implementing the merge rule `default_excludes ? DEFAULT_EXCLUDES : {} , + add_excludes, - remove_excludes`, split into directory names (trailing `/`) and file patterns (fnmatch, covering both exact names like `.DS_Store` and globs like `*.min.js`).
