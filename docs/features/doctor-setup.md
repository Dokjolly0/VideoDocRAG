...
- `core/utils/gpu.py` — side-effect-safe GPU detail probe for dedicated VRAM via NVML/`nvidia-smi`; shared GPU memory is intentionally excluded from planning.
...
## Tests
- Unit: `tests/core/test_cuda.py` (device count success/missing-package/call-failure, cuBLAS probe success/`OSError`, and a dedicated regression test asserting `winmode=0` is passed), `tests/core/test_gpu.py` (NVML/`nvidia-smi` probing and OOM detection), `tests/core/test_setup_actions.py` (success, `capture=False` threaded through, `CalledProcessError`/`OSError`), `tests/core/test_registry_service.py` (new: `last_load_was_corrupted` false/true/resets-on-next-clean-load, `registry_path`).
