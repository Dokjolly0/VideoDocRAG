...
## Summary
`videodoc ingest <project>` is README §15's "Fase 2 — Ingestion dei video": for every video file in `videos/` (internal or external — see `docs/features/external-source-paths.md`), it computes a content hash, probes duration/format/resolution/codec via `ffprobe`, registers the video in the project's own `project.db` SQLite database (README §31 schema), and creates a per-video `workdir/<id>/{audio,frames,transcript,ocr,chunks}/` plus a `metadata.json`. It is idempotent: an unchanged video is skipped without even being reprobed; a changed one is reprocessed. `ingest` is the first command allowed to require an external tool (`ffprobe`, part of FFmpeg) — `init`/`scan`/`list`/`link`/`unlink`/`path` remain dependency-free (README §37.8).

## What was implemented
- `core/utils/hashing.py::hash_file()` — streamed SHA-256, read in fixed-size blocks rather than loading the whole file at once (videos are routinely hundreds of MB to multiple GB).
...
- `core/storage/database.py` — a thin `sqlite3` wrapper (no ORM): `ensure_schema()` (`CREATE TABLE IF NOT EXISTS videos` per the exact README §31 schema — no `format`/`resolution`/`codec` columns; those, plus the `*_path` pointers, belong in each video's `metadata.json`, the same raw-vs-structured-data split already established by README §4.6), `get_video()`, `upsert_video()`. The upsert uses SQLite's `ON CONFLICT(id) DO UPDATE SET ...` and deliberately **excludes `title` and `created_at`** from the update clause — a reingest triggered by a changed hash must never clobber a title set elsewhere in the pipeline, nor overwrite the video's original first-ingested timestamp with the reprocessing time. All three functions wrap `sqlite3.Error` into a new `DatabaseError(VideoDocError)`, for structural DB failures (disk full, locked file) — distinct from a per-video probe/hash failure, which never aborts the whole run. Connections are opened via `contextlib.closing(sqlite3.connect(...))` chained with `sqlite3.Connection`'s own `with conn:` block: `sqlite3.Connection` used bare as a context manager only wraps commit/rollback, it does **not** close the connection on exit — a well-known stdlib gotcha that was actually caught here via a `ResourceWarning: unclosed database` surfaced by the test suite, not by inspection.
...
- `core/services/ingest_service.py::VideoIngestionService(project_dir, config).run() -> IngestResult` — the orchestration. Order of operations: resolve `videos/` and enumerate files (reusing `resolve_source_path`/`scan_videos`, the same helpers `scan` uses) → if zero video files, raise `NoVideosFoundError` → **then** check `ffprobe` availability (in that order, not the reverse — an empty project should be told about its own emptiness first, not asked to install FFmpeg for nothing to process yet) → `ensure_schema()` → per video (sorted for determinism): derive `video_id = slugify(stem)` (the same function project slugs use — one canonical identifier, not the three different derivations README's own §9.1/§15 illustrative example uses for "the video's name") → collision check → hash → skip if hash unchanged (without probing) → probe → write workdir + `metadata.json` + upsert DB row.
...
- `videodoc ingest` CLI: thin wrapper (`ProjectService.load()` → `VideoIngestionService(...).run()`), reports counts (`ingested`/`reingested`/`skipped`) plus the database file touched.

## Main files
- `src/videodoc/core/utils/hashing.py` — `hash_file`.
- `src/videodoc/core/utils/ffprobe.py` — `probe_video`, `VideoProbeResult`, `VideoProbeError`.
- `src/videodoc/core/storage/database.py` — `ensure_schema`, `get_video`, `upsert_video`, `VideoRow`.
...
- Idempotency is hash-based only, exactly as README §15 specifies — no `mtime`-based shortcut to skip rehashing an unmodified-looking file. Rehashing a multi-GB video on every run has a real cost, but a heuristic optimization not requested by the spec would add speculative complexity; the correct hash comparison is always performed.
...
## Tests
- Unit: `tests/core/test_hashing.py` (digest correctness, chunking invariance, missing file), `tests/core/test_ffprobe.py` (successful parse, `CalledProcessError`, malformed JSON, no video stream — all via a monkeypatched `subprocess.run`, no real `ffprobe` needed), `tests/core/test_database.py` (schema idempotency, insert/update roundtrip, `test_upsert_video_update_preserves_title_and_created_at`, `sqlite3.Error` wrapping for all three functions), `tests/core/test_video_metadata.py` (roundtrip, invalid JSON/missing file/extra field), `tests/core/test_filesystem.py` (`ensure_video_workdir` idempotency), `tests/core/test_ingest_service.py` (zero videos, missing ffprobe, single video, `test_unchanged_video_is_skipped_without_reprobing` — asserts a call-count of zero on `probe_video` for the second run, proving idempotency is real and not just a matching end state, reingest without workdir deletion, per-video hash/probe errors not blocking others, same-run and cross-run id collisions, external `paths.videos`).
