...
A video with no frames yet (`videodoc frames` never ran, or produced zero frames) is not an error — it's silently skipped. `contains_code`/`perceptual_hash` are never touched by this phase: `contains_code` is explicitly owned by the code-detection phase (`videodoc code`, README §20), and the frames table's own schema comment already documents this boundary.
...
- `contains_code`/`perceptual_hash` are never written by `update_frame_ocr` — enforced by construction (the SQL only ever sets `ocr_text`/`ocr_confidence`), not just by convention, so §20 code-detection results can never be silently wiped by a `videodoc ocr` re-run.
