...
- `core/utils/ocr_engine.py` — `rapidocr_available()` (cheap `find_spec` check, mirrors `scenedetect_available()`), `load_engine()` (one `RapidOCR()` instantiation, wraps any failure in `OCRRunError`), `run_ocr()` (single-image inference, joins `txts`, averages `scores`, handles the `None`/`None` no-detection case).
...
## Design decisions
- No `doctor` check for RapidOCR — follows the `scenedetect` precedent exactly: `scenedetect` is also a hard `pyproject.toml` dependency absent from `DoctorService.run()`'s fixed check list, its availability instead checked lazily inside the service that actually needs it, only when a run needs fresh work. RapidOCR follows the same pattern via `rapidocr_available()`.
